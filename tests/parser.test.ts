import { CardType } from '../src/model';
import { parseDecks as parse } from '../src/parser';

const defaultArgs = [true, true, true, '::', ':::', '?', '??'] as const;

test('Test parsing of single line basic cards', () => {
  expect(parse('Question::Answer', ...defaultArgs)).toEqual([[CardType.SingleLineBasic, 'Question::Answer', 0]]);
...
test('Test parsing of single line reversed cards', () => {
  expect(parse('Question:::Answer', ...defaultArgs)).toEqual([[CardType.SingleLineReversed, 'Question:::Answer', 0]]);
  expect(parse('Some text before\nQuestion :::Answer', ...defaultArgs)).toEqual([
...
test('Test parsing of cloze cards', () => {
  // ==highlights==
  expect(parse('cloze ==deletion== test', ...defaultArgs)).toEqual([[CardType.Cloze, 'cloze ==deletion== test', 0]]);
...
test('Test not parsing cards in HTML comments', () => {
  ...
  expect(
    parse('<!--\nQuestion\n?\nAnswer <!--SR:!2021-08-11,4,270-->\n\n<!--cloze ==deletion== test-->-->', ...defaultArgs)
  ).toEqual([]);
